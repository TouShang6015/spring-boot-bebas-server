...
public class RouteMenuVo {
     ...
     * 构建菜单路由
     */
    public RouteMenuVo(SysMenuDTO menu) {
...
//            this.children = menuChildren.stream().sorted(Comparator.comparing(SysMenuModel::getSort)).map(RouteMenuVo::new).collect(Collectors.toList());
            this.children = menuChildren.stream().map(item -> (SysMenuDTO) item).sorted(Comparator.comparing(SysMenuModel::getSort)).map(RouteMenuVo::new).collect(Collectors.toList());
        });
