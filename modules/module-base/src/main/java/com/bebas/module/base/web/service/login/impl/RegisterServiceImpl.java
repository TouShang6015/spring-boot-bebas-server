...
import com.org.bebasWh.utils.result.ResultUtil;
...
public class RegisterServiceImpl implements IRegisterService {
    ...
    public Result doRegister(@Valid RegisterBodyVo param) {
        ...
        if ( sysUserService.countByParam(SysUserDTO.builder().userName(param.getUserName()).build()) > 0 ){
            return Result.fail(MessageUtils.message("user.register.password.already"));
        }
        // 设置用户参数
        ...
        if (sysUserService.insertUser(_insertParam) > 0) {
            return Result.success(MessageUtils.message("user.register.success"));
        }
        return Result.fail(MessageUtils.message("user.register.fail"));
    }
}
